pub const JUDGE_REASONABLE: &str = "REASONABLE";
pub const MAX_JUDGE_ATTEMPTS: usize = 5;
pub const MAX_JUDGE_CLARIFICATION_RETRIES: usize = 2;
pub const MAX_GLOBAL_CYCLES: usize = 30;
pub const MAX_INSIST_ATTEMPTS: usize = 50;
pub const MAX_BRANCH_RETRIES: usize = 5;
pub const MAX_SPLIT_DEPTH: u32 = 3;
pub const DEFAULT_SOLVER_TIMEOUT_SECS: u64 = 60;