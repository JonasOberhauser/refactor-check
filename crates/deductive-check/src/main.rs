...
fn main() -> Result<()> {
    ...
    let bg_handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            // Forward gate errors to server state for status/continue protocols
            {
                ...
                let _gate_forwarder = std::thread::spawn(move || {
            ...
            let result: anyhow::Result<()> = rt.block_on(async move {
                ...
                let _v = bg_config.update(|cfg| cfg.llm.api_key = api_key.clone());
            ...
            match &result {
                Ok(()) => {
                    *bg_state.work_result.lock().expect("work result lock: single op") = None;
                }
                Err(e) => {
                    let msg = format!("{e:#}");
                    *bg_state.work_result.lock().expect("work result lock: single op") = Some(msg.clone());
    ...
    let _error_forwarder = std::thread::spawn(move || {
    ...
    let _prev_epoch = server_state.epoch.fetch_add(1, Ordering::Release);

    // Wait for bg thread
    let (done_tx, done_rx) = std::sync::mpsc::channel();
    let _awaiter = std::thread::spawn(move || {
        let _ = done_tx.send(bg_handle.join());
    });
    match done_rx.recv_timeout(std::time::Duration::from_secs(2)) {
        Ok(Ok(())) => {
            if let Some(err) = server_state.work_result.lock().expect("work result lock: single op").take() {
