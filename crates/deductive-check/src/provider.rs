...
impl IOProvider<PythonRequest, PythonResponse> for ProcessPythonProvider {
    async fn invoke(&self, input: PythonRequest) -> Result<PythonResponse> {
        loop {
            ...
            let message = format!(
                "python3 failed (exit {:?}): {} — the script was generated by the formalizer/fixer LLM, so this is an LLM-recoverable error that must be fixed by the fixer, not a human",
                output.status.code(),
                stderr,
            );
            warn!(error = %stderr, "python3 script failed");
            anyhow::bail!("{message}");
        }
    }
}
